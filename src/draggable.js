...
export class Draggable {
   ...
   onmousemove(e) {
      ...
      for (const dt of me.dropTargets) {
         ...
         if (oldPs !== undefined && oldPs !== ps) {
            const states = Object.keys(azdom.dndState);
            for (const state of states) {
               const nState = ps & azdom.dndState[state];
               const oState = oldPs & azdom.dndState[state];
               if (nState !== oState) {
                  const eventName = state + (!!nState ? '_in' : '_out');
                  if (azdom.dndEvent[eventName] & dt.interestedDropEvents) {
                     dt.dt.dispatchEvent(
   ...
   onmouseup(e) {
      ...
      me.dropTargets.map(dt => {
         // const dropId = dt.getAttribute('drop-id');
         const ps = azdom.getPositionState(dom, dt.dt, e);
         if (azdom.dndEvent.dropped & dt.interestedDropEvents) {
            dt.dt.dispatchEvent(
   ...
   onmousedown(e) {
      ...
      me.dropTargets = [...document.querySelectorAll('.azui-droppable')]
         ...
         .map(dt => {
            ...
            const ps = azdom.getPositionState(dom, ret.dt, e);
            me.dropTargetStates[ret.dropId] = ps;
